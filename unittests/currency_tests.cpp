...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         // Modified by wschoi
         return get_currency_balance(N(osbio.token), symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         // Modified by wschoi
         auto trace = push_action(N(osbio.token), N(issue), mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      {
         // Modified by wschoi
         create_account( N(osbio.token));
         set_code( N(osbio.token), contracts::eosio_token_wasm() );

         // Modified by wschoi
         auto result = push_action(N(osbio.token), N(create), mutable_variant_object()
                 ("issuer",       eosio_token)
         ...
         // Modified by wschoi
         result = push_action(N(osbio.token), N(issue), mutable_variant_object()
                 ("to",       eosio_token)
...
// Modified by wschoi
const std::string currency_tester::eosio_token = name(N(osbio.token)).to_string();
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   // Modified by wschoi
   auto actual = t.get_currency_balance(N(osbio.token), expected.get_symbol(), N(osbio.token));
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      // Modified by wschoi
      auto trace = push_action(N(osbio.token), N(transfer), mutable_variant_object()
         ("from", eosio_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   // Modified by wschoi
   BOOST_REQUIRE_THROW(push_action(N(osbio.token), N(transfer), mutable_variant_object()
                                    ("from", eosio_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
