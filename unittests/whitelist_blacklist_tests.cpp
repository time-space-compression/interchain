...
// Modified by wschoi
         chain->create_accounts({N(osbio.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(osbio.token), contracts::eosio_token_wasm() );
         chain->set_abi(N(osbio.token), contracts::eosio_token_abi().data() );
         chain->push_action( N(osbio.token), N(create), N(osbio.token), mvo()
              ( "issuer", "osbio.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );

         // Modified by wschoi
         chain->push_action( N(osbio.token), N(issue), N(osbio.token), mvo()
              ( "to", "osbio.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         // Modified by wschoi
         return chain->push_action( N(osbio.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   // Modified by wschoi
   test.actor_whitelist = {config::system_account_name, N(osbio.token), N(alice)};
   test.init();

// Modified by wschoi
   test.transfer( N(osbio.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   // Modified by wschoi
   test.contract_whitelist = {config::system_account_name, N(osbio.token), N(bob)};
   test.init();

// Modified by wschoi
   test.transfer( N(osbio.token), N(alice), "1000.00 TOK" );
   test.transfer( N(alice), N(osbio.token) );
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   // Modified by wschoi
   test.contract_whitelist = {config::system_account_name, N(osbio.token), N(bob), N(charlie)};
