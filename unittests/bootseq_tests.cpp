...
public:
   ...
   fc::variant get_global_state() {
    ...
    auto register_producer(name producer) {
       ...
       auto r = base_tester::push_action(config::system_account_name, N(reggen), producer, mvo()
    ...
    asset get_balance( const account_name& act ) {
         ...
         // Modified by wschoi
         return get_currency_balance(N(osbio.token), symbol(CORE_SYMBOL), act);
         // Origin
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Modified by wschoi
        create_accounts({N(eosio.msig), N(osbio.token), N(eosio.ram), N(eosio.ramfee), N(eosio.stake), N(eosio.vpay), N(osbio.bpay), N(osbio.saving) });
        // Origin
        ...
        // Modified by wschoi
        const auto& eosio_token_acc = get<account_metadata_object, by_name>(N(osbio.token));
        // Origin
        ...
        // Modified by wschoi
        create_currency(N(osbio.token), config::system_account_name, max_supply);
        // Origin
        ...
        // Modified by wschoi
        issue(N(osbio.token), config::system_account_name, config::system_account_name, initial_supply);
        // Origin
