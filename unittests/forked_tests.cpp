...
// Modified by wschoi
   auto r2 = c.create_accounts( {N(osbio.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(osbio.token), contracts::eosio_token_wasm() );
   c.set_abi( N(osbio.token), contracts::eosio_token_abi().data() );
   c.produce_blocks(10);

// Modified by wschoi
   auto cr = c.push_action( N(osbio.token), N(create), N(osbio.token), mutable_variant_object()
              ("issuer",       "eosio" )
              ("maximum_supply", core_from_string("10000000.0000"))
      );

// Modified by wschoi
   cr = c.push_action( N(osbio.token), N(issue), config::system_account_name, mutable_variant_object()
              ("to",       "eosio" )
...
// Modified by wschoi
   cr = c.push_action( N(osbio.token), N(transfer), config::system_account_name, mutable_variant_object()
              ("from",     "eosio")
              ("to",       "dan" )
              ("quantity", core_from_string("100.0000"))
              ("memo", "")
