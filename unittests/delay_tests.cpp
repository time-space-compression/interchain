...
asset get_currency_balance(const TESTER& chain, account_name account) {
   // Modified by wschoi
   return chain.get_currency_balance(N(osbio.token), symbol(SY(4,CUR)), account);
}

// Modified by wschoi
const std::string eosio_token = name(N(osbio.token)).to_string();
...
BOOST_AUTO_TEST_CASE( link_delay_direct_test ) { try {
   ...
   // Modified by wschoi
   chain.create_account(N(osbio.token));
   chain.produce_blocks(10);

   // Modified by wschoi
   chain.set_code(N(osbio.token), contracts::eosio_token_wasm());
   chain.set_abi(N(osbio.token), contracts::eosio_token_abi().data());
   ...
   // Modified by wschoi
   chain.push_action(N(osbio.token), N(create), N(osbio.token), mutable_variant_object()
           ("issuer", eosio_token)
           ("maximum_supply", "9000000.0000 CUR")
   );

   // Modified by wschoi
   chain.push_action(N(osbio.token), name("issue"), N(osbio.token), fc::mutable_variant_object()
           ("to",       eosio_token)
   ...
   // Modified by wschoi
   auto trace = chain.push_action(N(osbio.token), name("transfer"), N(osbio.token), fc::mutable_variant_object()
       ("from", eosio_token)
       ("to", "tester")
   ...
   // Modified by wschoi
   auto liquid_balance = get_currency_balance(chain, N(osbio.token));
   ...
   // Modified by wschoi
   trace = chain.push_action(N(osbio.token), name("transfer"), N(tester), fc::mutable_variant_object()
       ("from", "tester")
       ("to", "tester2")
       ("quantity", "1.0000 CUR")
       ("memo", "hi" )
   ...
   // Modified by wschoi
   liquid_balance = get_currency_balance(chain, N(osbio.token));
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = get_currency_balance(chain, N(tester));
...
BOOST_AUTO_TEST_CASE(delete_auth_test) { try {
   ...
   // Modified by wschoi
   trace = chain.push_action(N(osbio.token), name("transfer"), N(osbio.token), fc::mutable_variant_object()
       ("from", "osbio.token")
       ("to", "tester")
       ("quantity", "100.0000 CUR")
       ("memo", "hi" )
...
// Modified by wschoi
   ...
   // Modified by wschoi
   const auto& acnt = chain.control->db().get<account_object,by_name>(N(osbio.token));
   const auto abi = acnt.get_abi();
   chain::abi_serializer abis(abi, chain.abi_serializer_max_time);
   const auto a = chain.control->db().get<account_object,by_name>(N(osbio.token)).get_abi();
...
// Modified by wschoi
...

   {
      ...
      // Modified by wschoi
      trace = chain.push_action(N(osbio.token), name("transfer"), vector<permission_level>{{N(tester), N(first)}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "1.0000 CUR")
          ("memo", "hi" ),
   ...
   ilog("attempting second delayed transfer");
   {
      // this transaction will be delayed 10 blocks
      // Modified by wschoi
      trace = chain.push_action(N(osbio.token), name("transfer"), vector<permission_level>{{N(tester), N(second)}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "5.0000 CUR")
          ("memo", "hi" ),
...

   {
      // this transaction will be delayed 10 blocks
      // Modified by wschoi
      trace = chain.push_action(N(osbio.token), name("transfer"), vector<permission_level>{{N(tester), config::owner_name}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "10.0000 CUR")
          ("memo", "hi" ),
