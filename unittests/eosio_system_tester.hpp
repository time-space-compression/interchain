...
public:
   ...
   eosio_system_tester(Lambda setup) {
      ...
      // Modified by wschoi
      create_currency( N(osbio.token), config::system_account_name, core_from_string("10000000000.0000") );
   ...
   asset get_balance( const account_name& act ) {
      // Modified by wschoi
      vector<char> data = get_row_by_account( N(osbio.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
