...
def regGens(b, e):
    for i in range(b, e):
        a = accounts[i]
        # regproducer modified by wschoi
        retry(args.cleos + 'system reggen ' + a['name'] + ' ' + a['pub'] + ' https://' + a['name'] + '.com' + '/' + a['pub'])
        # Origin
...
def listProducers():
    ...
    run(args.cleos + 'system listgenerators')
    # Origin
...
def stepInstallSystemContracts():
    ...
    # Modified by wschoi
    run(args.cleos + 'set contract osbio.token ' + args.contracts_dir + '/osbio.token/')
    # Origin
...
def stepCreateTokens():
    ...
    # Modified by wschoi
     run(args.cleos + 'push action osbio.token create \'["eosio", "10000000000.0000 %s"]\' -p osbio.token' % (args.symbol))
    totalAllocation = allocateFunds(0, len(accounts))
    run(args.cleos + 'push action osbio.token issue \'["eosio", "%s", "memo"]\' -p eosio' % intToCurrency(totalAllocation))
    sleep(1)
    # Origin
...
def stepRegProducers():
    # regproducer modified by wschoi
    regGens(firstProducer, firstProducer + numProducers)
    # Origin
