...
//hdkim
      const auto token_code = N(osbio.token);
...
chain::symbol read_only::extract_core_symbol()const {
   ...
   //const auto* t_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple( N(osbio), N(osbio), N(rammarket) ));
   if( t_id != nullptr ) {
      const auto &idx = d.get_index<key_value_index, by_scope_primary>();
