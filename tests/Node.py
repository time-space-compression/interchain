...
class Node(object):
    ...
    def getAccountEosBalanceStr(self, scope):
        ...
        assert isinstance(scope, str)
        #hdkim
        amount=self.getTableAccountBalance("osbio.token", scope)
    ...
    def reggen(self, producer, url, location, waitForTransBlock=False, exitOnError=False):
        cmdDesc="system reggen"
        cmd="%s -j %s %s %s %s" % (
