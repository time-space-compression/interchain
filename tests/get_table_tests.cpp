...
// Modified by wschoi
              std::string memo = "", account_name token_contract = N(osbio.token) )
// Origin
...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   // Modified by wschoi
   create_accounts({ N(osbio.token), N(eosio.ram), N(eosio.ramfee), N(eosio.stake),
      N(osbio.bpay), N(eosio.vpay), N(osbio.saving), N(eosio.names) });
   // Origin
   ...
   set_abi( N(osbio.token), contracts::eosio_token_abi().data() );
   // Origin
   ...
   // Modified by wschoi
   push_action(N(osbio.token), N(create), N(osbio.token), act );
   // Origin
   ...
   // Modified by wschoi
   eosio::chain_apis::read_only::get_table_by_scope_params param{N(osbio.token), N(accounts), "inita", "", 10};
   // Origin
   ...
   if (result.rows.size() >= 4) {
      ...
      // Modified by wschoi
      BOOST_REQUIRE_EQUAL(name(N(osbio.token)), result.rows[0].code);
      // Origin
