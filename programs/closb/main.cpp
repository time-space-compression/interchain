...
authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   //hdkim
   if (boost::istarts_with(authorityJsonOrFile, "OSB") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
...
inline asset to_asset( const string& s ) {
   ...
   return to_asset( N(osbio.token), s );
}
...
void ensure_keosd_running(CLI::App* app) {
    ...
    if (auto* subapp = app->get_subcommand("system")) {
       ...
       // listproducers modified by wschoi
       if (subapp->got_subcommand("listgenerators") || subapp->got_subcommand("listbw") || subapp->got_subcommand("bidnameinfo")) // system list* do not require wallet
         return;
    }
...
struct register_producer_subcommand {
   ...
   register_producer_subcommand(CLI::App* actionRoot) {
      ...
      auto register_producer = actionRoot->add_subcommand("reggen", localized("Register a new generator"));
      // Origin
      ...
      register_producer->set_callback([this] {
         ...
         // regproducer modified by wschoi
         send_actions({create_action(accountPermissions, config::system_account_name, N(reggen), regprod_var)});
         // Origin
...
struct unregister_producer_subcommand {
   ...
   unregister_producer_subcommand(CLI::App* actionRoot) {
      ...
      auto unregister_producer = actionRoot->add_subcommand("unreggen", localized("Unregister an existing generator"));
      // Origin
      ...
      unregister_producer->set_callback([this] {
         ...
         // unregprod modified by wschoi
         send_actions({create_action(accountPermissions, config::system_account_name, N(unreggen), act_payload)});
         // Origin
...
struct vote_producer_proxy_subcommand {
   ...
   vote_producer_proxy_subcommand(CLI::App* actionRoot) {
      ...
      vote_proxy->set_callback([this] {
         ...
         send_actions({create_action(accountPermissions, config::system_account_name, N(votegen), act_payload)});
         // Origin
...
struct list_producers_subcommand {
   ...
   list_producers_subcommand(CLI::App* actionRoot) {
      ...
      // listproducers modified by wschoi
      auto list_producers = actionRoot->add_subcommand("listgenerators", localized("List generators"));
      // Origin
...
//                                ->check(CLI::ExistingFile);
   ...
   // voteproducer modified by wschoi
   auto voteProducer = system->add_subcommand("votegen", localized("Vote for a generator"));
   // Origin
